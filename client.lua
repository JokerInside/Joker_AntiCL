...
RegisterNetEvent("Joker_AntiCL:show")
AddEventHandler("Joker_AntiCL:show", function()
    if show3DText then
        show3DText = true
    else
        show3DText = true
...
RegisterNetEvent("Joker_AntiCL")
AddEventHandler("Joker_AntiCL", function(id, crds, identifier, reason)
    ...
    Citizen.CreateThread(function()
        while displaying do
            ...
            if GetDistanceBetweenCoords(crds.x, crds.y, crds.z, pcoords.x, pcoords.y, pcoords.z, true) < 15.0 and show3DText then
                DrawText3DSecond(crds.x, crds.y, crds.z+0.15, "People leave town!")
                DrawText3D(crds.x, crds.y, crds.z, "Pocket: "..id.." ["..identifier.."]\nBecause: "..reason)
            else
                Citizen.Wait(2000)
            end
        end
